...
abstract contract StratX2 is Ownable, ReentrancyGuard, Pausable {
    ...
    modifier onlyPathSetter() {
        require(msg.sender == pathSetter, "!pathSetter");
        _;
    }
    ...
    function changePathSetter(address newAddress) public onlyAllowGov {
        pathSetter = newAddress;
    }
    function SetearnedToTENFIPath(address[] memory newPath) public onlyPathSetter {
        earnedToTENFIPath = newPath;
    }
    function SetearnedToToken0Path(address[] memory newPath) public onlyPathSetter {
        earnedToToken0Path = newPath;
    }
    function SetearnedToToken1Path(address[] memory newPath) public onlyPathSetter {
        earnedToToken1Path = newPath;
    }
    function Settoken0ToEarnedPath(address[] memory newPath) public onlyPathSetter {
        token0ToEarnedPath = newPath;
    }
    function Settoken1ToEarnedPath(address[] memory newPath) public onlyPathSetter {
        token1ToEarnedPath = newPath;
    }
        ...
        returns (uint256)
    {
        ...
        if (isTENFIComp) {            
            uint256 bal = IERC20(earnedAddress).balanceOf(address(this));
            _farm();
            uint256 earnedAmt = IERC20(earnedAddress).balanceOf(address(this)).sub(bal);
